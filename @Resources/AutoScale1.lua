...
function ApplyScale()
    ...
    if override == 1 then
        -- OverrideAutoScale is ON: keep manual ScaleDpi, do NOT move any widgets
        scale = tonumber(SKIN:GetVariable('ScaleDpi')) or 1
        SKIN:Bang('!Log "AutoScale1: OverrideAutoScale=1, skipping position reset"')
        return
    else
    ...
    -- Refresh only active skins via group, avoids warnings on inactive skins
    SKIN:Bang('!RefreshGroup', 'WidgetsLight')
    SKIN:Bang('!RefreshGroup', 'WidgetsDark')
